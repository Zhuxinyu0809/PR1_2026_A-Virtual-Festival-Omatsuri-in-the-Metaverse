...
//#endif
	...
	//!!!!!!!!!!!!make color white for additional light calculations to get the color correctly
#ifdef _AKMU_SHADOW_RECEIVER_SHOWLIGHTS
    surfaceData.albedo = half3(1, 1, 1);
#endif //_AKMU_SHADOW_RECEIVER_SHOWLIGHTS
	
#if defined(_SPECULARHIGHLIGHTS_OFF)
    bool specularHighlightsOff = true;
#else //defined(_SPECULARHIGHLIGHTS_OFF)
    bool specularHighlightsOff = false;
#endif //defined(_SPECULARHIGHLIGHTS_OFF)
    BRDFData brdfData;
    // NOTE: can modify "surfaceData"...
    InitializeBRDFData(surfaceData, brdfData);
	//get main light atten and other setup
	...
	//calculate the main light atten
    half alphaMain = 1 - mainLight.shadowAttenuation;
    half alphaAdditional1 = 1;
    half alphaAdditional2 = 1;
    half alphaAdditional3 = 1;
    half alphaAdditional4 = 1;
    half alphaAdditional5 = 1;
    half alphaAdditional6 = 1;
    half alphaAdditional7 = 1;
    half alphaAdditional8 = 1;
	...
	if(pixelLightCount == 5)
	{
		alphaAdditional6 = 0;
		alphaAdditional7 = 0;
		alphaAdditional8 = 0;
	}
	if(pixelLightCount == 6)
	{
		alphaAdditional7 = 0;
		alphaAdditional8 = 0;
	}
	if(pixelLightCount == 7)
	{
		alphaAdditional8 = 0;
	}
	...
	LIGHT_LOOP_BEGIN(pixelLightCount)
        Light light = GetAdditionalLight(counter, inputData, shadowMask, aoFactor);
	
	 //   float4 lightPositionWS = _AdditionalLightsPosition[counter];
		//// Directional lights store direction in lightPosition.xyz and have .w set to 0.0.
		//// This way the following code will work for both directional and punctual lights.
		//float3 lightVector = lightPositionWS.xyz - inputData.positionWS * lightPositionWS.w;
		//float distanceSqr = max(dot(lightVector, lightVector), HALF_MIN);
		//float d = distanceSqr * _AdditionalLightsColorDistance;
	
		//float d = distance(input.positionWS.xyz, _AdditionalLightsPosition[counter].xyz) * _AdditionalLightsColorDistance;
		float d = 0; //abs(distance(input.positionWS.xyz, _AdditionalLightsAttenuation[counter].xyz)) * _AdditionalLightsColorDistance;
		//float d = _AdditionalLightsColorDistance;

		if(counter == 0)
		{
			alphaAdditional1 -= light.shadowAttenuation;
			alphaAdditional1 -= d;
			alphaAdditional1 = clamp(0,alphaAdditional1,1);
		}
		if(counter == 1)
		{
			alphaAdditional2 -= light.shadowAttenuation;
			alphaAdditional2 -= d;
			alphaAdditional2 = clamp(0,alphaAdditional2,1);
		}
		if(counter == 2)
		{
			alphaAdditional3 -= light.shadowAttenuation;
			alphaAdditional3 -= d;
			alphaAdditional3 = clamp(0,alphaAdditional3,1);
		}
		if(counter == 3)
		{
			alphaAdditional4 -= light.shadowAttenuation;
			alphaAdditional4 -= d;
			alphaAdditional4 = clamp(0,alphaAdditional4,1);
		}
		if(counter == 4)
		{
			alphaAdditional5 -= light.shadowAttenuation;
			alphaAdditional5 -= d;
			alphaAdditional5 = clamp(0,alphaAdditional5,1);
		}
		if(counter == 5)
		{
			alphaAdditional6 -= light.shadowAttenuation;
			alphaAdditional6 -= d;
			//alphaAdditional6 = clamp(0,alphaAdditional6,1);
		}
		if(counter == 6)
		{
			alphaAdditional7 -= light.shadowAttenuation;
			alphaAdditional7 -= d;
			alphaAdditional7 = clamp(0,alphaAdditional7,1);
		}
		if(counter == 7)
		{
			alphaAdditional8 -= light.shadowAttenuation;
			alphaAdditional8 -= d;
			alphaAdditional8 = clamp(0,alphaAdditional8,1);
		}
...
#endif // _LIGHT_LAYERS
        {
			...
			//lightingData.additionalLightsColor += light.color;
#endif //_AKMU_SHADOW_RECEIVER_SHOWLIGHTS
        }
	counter+=1;
    LIGHT_LOOP_END
#endif //defined(_ADDITIONAL_LIGHTS)
	
	//test
    //outColor = half4(lightingData.additionalLightsColor, 1);
    //return;
	
	//for texture mapping on shadow :)
    half4 albedoAlpha = SampleAlbedoAlpha(input.uv, TEXTURE2D_ARGS(_BaseMap, sampler_BaseMap));
	
	half totalAlphaAdditional = alphaAdditional1 + alphaAdditional2 + alphaAdditional3 + alphaAdditional4 + 
		alphaAdditional5 + alphaAdditional6 + alphaAdditional7 + alphaAdditional8;
	totalAlphaAdditional = clamp(0,totalAlphaAdditional,1);
	
#ifdef _AKMU_SHADOW_RECEIVER_SHOWLIGHTS
    half additionaLightGrayScale = dot(lightingData.additionalLightsColor, half4(0.22, 0.707, 0.071, 0.0).rgb);
	
	outColor = half4(albedoAlpha.rgb * _BaseColor.rgb + lightingData.additionalLightsColor, 
		(totalAlphaAdditional + alphaMain + additionaLightGrayScale) * _BaseColor.a);
#else // _AKMU_SHADOW_RECEIVER_SHOWLIGHTS
    outColor = half4(albedoAlpha.rgb * _BaseColor.rgb,
		(totalAlphaAdditional + alphaMain) * _BaseColor.a);
#endif // _AKMU_SHADOW_RECEIVER_SHOWLIGHTS
    return;
